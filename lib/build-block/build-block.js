...
var autominApi = {
	parse: function(src, dest, options) {
		...
		for (var i in blocks) {
			...
			var tasks = minTypes.processPipeline(gruntTargetId, blocks[i].type, blocks[i].content, newSrcDir, destDir, blocks[i].target, options, pageOptions, blocks[i].options);
